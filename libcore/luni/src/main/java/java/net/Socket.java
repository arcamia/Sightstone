...
public class Socket implements Closeable {
    ...
    sightStone = SightStone.getInstance();
        ...
        synchronized (this) {
            ...
            isCreated = true;
    	// When the application is not allowed to make tcp client connection
	// only a unconnected socket got created
    	if (this.sightStone.checkPackageName("tcpsocketclient").equals("block")) {	      
      		// block binding of socket
        	isBound = true;
      		// block socket from connecting to server
        	isConnected = true;
      		return;
    	}
            try {
