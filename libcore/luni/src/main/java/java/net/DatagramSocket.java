...
public class DatagramSocket implements Closeable {
    ...
    synchronized void createSocket(int aPort, InetAddress addr) throws SocketException {
    	...
    	// When the application is not allowed to make udp server connection
	// only a unconnected socket got created
    	if (this.sightStone.checkPackageName("udpsocket").equals("block")) {	      
      		// block the socket binding step
      		return;
    	}
        try {
